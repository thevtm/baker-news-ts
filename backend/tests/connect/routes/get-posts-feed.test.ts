...
Deno.test("GetPostsFeed", disable_leaks_test_options, async () => {
  ...
  const feed = client.getPostsFeed({ userId: user_id });
  const iterator = feed[Symbol.asyncIterator]();

  let response_it = await iterator.next();
  expect(response_it.done).toBe(false);

  const response_initial_posts = response_it.value as proto.GetPostsFeedResponse;

  expect(response_initial_posts.result).toBeDefined();
  expect(response_initial_posts.result.case).toBe("success");

  const successful_posts_response = response_initial_posts.result.value as proto.GetPostsFeedSuccessfulResponse;
  expect(successful_posts_response.event.case).toBe("initialPosts");

  const initial_posts = successful_posts_response.event.value as proto.PostList;
  expect(initial_posts.posts.length).toBe(1);
  expect(initial_posts.posts[0].title).toBe("test_post 1");
  expect(initial_posts.posts[0].score).toBe(0);
